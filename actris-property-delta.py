...
templateUrl = Variable.get("actris_prop_delta_sold_index_url")
...
def createIndexSpec(templateContent, dataSource, intervals):
  baseDir = '/var/shared-data/actris-delta'
  template = replace(templateContent, baseDir, dataSource, intervals)

  return template

with DAG(
    dag_id='actris-property-delta',
    ...
    load = KubernetesPodOperator(namespace='data',
                ...
                cmds=["sh","-c", "dotnet DataLoader.dll '{link}' '/shared-data' 'actris-delta'".format(link=url)],
