...
harUrl = Variable.get("har_mls_validation_url")
templateUrl = Variable.get("har_mls_validation_index_url")
harPropDataSource = Variable.get("har_mls_validation_datasource")
...
def createIndexSpec(templateContent, dataSource, intervals):
  baseDir = '/var/shared-data/har-mls-validation'
  template = replace(templateContent, baseDir, dataSource, intervals)

  return template

with DAG(
    dag_id='har-mls-validation',
    ...
    load = KubernetesPodOperator(namespace='data',
                ...
                cmds=["sh","-c", "dotnet DataLoader.dll '{link}' '/shared-data' 'har-mls-validation'".format(link=url)],
