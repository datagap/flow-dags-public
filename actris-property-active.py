...
actrisUrl = Variable.get("actris_prop_active_url")

templateUrl = Variable.get("actris_prop_active_index_url")
actrisPropDataSource = Variable.get("actris_prop_active_datasource")
...
def createIndexSpec(templateContent, dataSource):
  baseDir = '/var/shared-data/actris-active'
  template = replace(templateContent, baseDir, dataSource)

  return template

with DAG(
    dag_id='actris-property-active',
    ...
    indexSpec = createIndexSpec(templateContent, actrisPropDataSource)
    ...
    load = KubernetesPodOperator(namespace='data',
                ...
                cmds=["sh","-c", "dotnet DataLoader.dll '{link}' '/shared-data' 'actris-active'".format(link=actrisUrl)],
