...
actrisUrl = Variable.get("actris_druid_validation_url")
templateUrl = Variable.get("actris_druid_validation_index_url")
actrisPropDataSource = Variable.get("actris_druid_validation_datasource")
...
def createIndexSpec(templateContent, dataSource, intervals):
  baseDir = '/var/shared-data/actris-druid-validation'
  template = replace(templateContent, baseDir, dataSource, intervals)

  return template

with DAG(
    dag_id='actris-druid-validation',
    ...
    load = KubernetesPodOperator(namespace='data',
                ...
                cmds=["sh","-c", "dotnet DataValidation.dll '{link}' '/shared-data' 'actris-druid-validation' '{startTime}' '{endTime}'".format(link=url, startTime=yesterday, endTime=today)],
