...
harUrl = Variable.get("har_druid_validation_url")
templateUrl = Variable.get("har_druid_validation_index_url")
harPropDataSource = Variable.get("har_druid_validation_datasource")
...
def createIndexSpec(templateContent, dataSource, intervals):
  baseDir = '/var/shared-data/har-druid-validation'
  template = replace(templateContent, baseDir, dataSource, intervals)

  return template

with DAG(
    dag_id='har-druid-validation',
    default_args=default_args,
    schedule_interval="0 6 * * *",
    start_date=days_ago(2),
    tags=['har', 'druidvalidation'],
) as dag:
    ...
    load = KubernetesPodOperator(namespace='data',
                image="datagap/datavalidation:latest",
                image_pull_policy='Always',
                cmds=["sh","-c", "dotnet DataValidation.dll '{link}' '/shared-data' 'har-druid-validation' '{startTime}' '{endTime}'".format(link=url, startTime=yesterday, endTime=today)],
                task_id="load-druid-validation-task-" + str(yesterday),
                name="load-druid-validation-task-" + str(yesterday),
                volumes=[volume],
