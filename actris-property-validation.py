...
actrisUrl = Variable.get("actris_prop_delta_url")
...
with DAG(
    dag_id='actris-property-validation',
    default_args=default_args,
    schedule_interval="0 8 * * *",
    start_date=days_ago(2),
    tags=['actris', 'validation'],
) as dag:
    ...
    druidQuery = '{"query":"SELECT ListingId, __time, ListOfficeName FROM \"{datasource}\" WHERE \"__time\" BETWEEN TIMESTAMP \'{yesterday}\' AND TIMESTAMP \'{today}\'"}'.format(datasource=actrisPropDatasource, yesterday=yesterday, today=today)
    ...
    loadMls = KubernetesPodOperator(namespace='data',
                ...
                image_pull_policy='Always',
                cmds=["sh","-c", "dotnet DataLoader.dll '{link}' '/shared-data' 'actris-validation'".format(link=mlsUrl)],
